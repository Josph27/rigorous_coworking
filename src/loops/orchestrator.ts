...
export function setupOrchestrator(pi: ExtensionAPI): void {
	...
	// ── Detect API errors and model looping ──
	pi.on("after_provider_response", async (event, _ctx) => {
		...
		if (event.status >= 400) {
			state._apiErrors++;
			console.error(
				`[rigor] API error ${event.status} (count: ${state._apiErrors})`,
			);
		}
	});
	...
	pi.on("agent_end", async (event, ctx) => {
		...
		// ── API error bailout ──
		if (state._apiErrors >= 3) {
			// phase captured in state.phase
			state._apiErrors = 0;
			state.phase = "plan_review";
			ctx.ui.notify(
				"API errors detected (3+). Pausing. Review and /greenlight to continue.",
				"error",
			);
			return;
		}
		...
		if (text) {
			state._lastAssistantTexts.push(text);
			if (state._lastAssistantTexts.length > 5)
				state._lastAssistantTexts.shift();
			if (state._lastAssistantTexts.length >= 3) {
				const last3 = state._lastAssistantTexts.slice(-3);
				const unique = new Set(last3.map((t) => t.slice(0, 200)));
				if (unique.size <= 1) {
					state._consecutiveLoops++;
					if (state._consecutiveLoops >= 2) {
						state._consecutiveLoops = 0;
						state._lastAssistantTexts = [];
						state.phase = "plan_review";
						ctx.ui.notify(
							"Loop detected (repeating output). Execution paused. Review and /greenlight.",
							"error",
						);
						return;
					}
		...
		// ── Dead-turn detection: LLM never produced output ──
		const deadPhases = ["ombudsman_analyzing", "planning"];
		if (deadPhases.includes(state.phase)) {
			const hasAssistant = event.messages.some(
				(m) =>
					m.role === "assistant" ||
			...
			const hasToolCall =
				...
				(state.planGraph && state.planGraph.steps.length > 0);
			if (!hasAssistant && !hasToolCall) {
				state._consecutiveFailures++;
				console.error(
					`[rigor] Dead turn in ${state.phase} (failures: ${state._consecutiveFailures})`,
				);
				if (state._consecutiveFailures >= 3) {
					state._consecutiveFailures = 0;
					state.phase = "plan_review";
					ctx.ui.notify(
						"LLM model unavailable (3 dead turns). Check model config. /greenlight to retry.",
						"error",
					);
					return;
				}
...
async function handlePlanningPhase(
	...
	if (!plan || plan.steps.length === 0) {
		...
		// Bail if we retried too many times without the LLM producing a plan
		state._consecutiveFailures++;
		if (state._consecutiveFailures >= 3) {
			state._consecutiveFailures = 0;
			state.phase = "plan_review";
			ctx.ui.notify(
				"Plan Designer: LLM unresponsive after 3 attempts. Check model config. /greenlight to retry.",
				"error",
			);
			return;
		}
...
async function handleExecutionPhase(
	...
	const stepFailed = responseText
		? detectExceptionInResponse(responseText)
		: false;
...
function triggerPlanDesigner(pi: ExtensionAPI, ctx: ExtensionContext): void {
	...
	const toolDefinitions = allTools
		.filter(
			(t) => t.name !== "mutate_plan_graph" && t.name !== "submit_gap_analysis",
		)
 ...
 */
function extractLastAssistantText(messages: readonly unknown[]): string | null {
...
async function handleMode2Exception(
	...
	const state = getState();
	const proposedSteps = (
		assistantText.match(/\b(?:new step|sub-step|additional step)\b/gi) || []
	).length;
...
export function updateStatusWidget(ctx: {
	...
	if (!state.activeTask) {
		...
		ctx.ui.setWidget("rigorous-task-banner", undefined as unknown as string[]);
		return;
	}
